 ...
 * @head: Pointer to pointer to a struct of type stack_t
...
void push(stack_t **head, unsigned int line_number)
{
	...
	if (helpy.queueflag == 1)
		result = queuepush(&helpy.line_number);
	if (result == 0)
		return;
	if (!head)
	{
 ...
 * @head: Pointer to a struct of tpe stack_t
...
/**
 * free_dlistintt - free a dlistint_t list
...
/**
 * sw - swaps the to two elements of the stack
 * @head: pointer to pointer to struct of type stack_t
 * @line_number: line number of instruction
 * Return: void
 */
