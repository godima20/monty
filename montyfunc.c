...
void monty_push(stack_t **stack, unsigned int line_number)
{
	...
	if (new == NULL)
	{
		set_op_tok_error(malloc_errors());
		return;
	}

	if (op_toks[1] == NULL)
	{
		set_op_tok_error(no_int_errors(line_number));
		return;
	}
...
void monty_pint(stack_t **stack, unsigned int line_number)
{
	if ((*stack)->next == NULL)
	{
		set_op_tok_error(pint_errors(line_number));
		return;
	}
...
void monty_pop(stack_t **stack, unsigned int line_number)
{
	...
	if ((*stack)->next == NULL)
	{
		set_op_tok_error(pop_errors(line_number));
		return;
	}
...
void monty_swap(stack_t **stack, unsigned int line_number)
{
	...
	{
		set_op_tok_error(short_stack_errors(line_number, "swap"));
		return;
	}
